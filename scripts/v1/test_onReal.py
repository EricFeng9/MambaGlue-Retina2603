...
# 导入真实数据集
from data.operation_pre_filtered_cffa.operation_pre_filtered_cffa_dataset import CFFADataset as CFFADataset_Val
...
# ==========================================
# 核心模型: PL_MambaGlue_Real
# ==========================================
class PL_MambaGlue_Real(pl.LightningModule):
    """MambaGlue 的 PyTorch Lightning 封装（用于真实数据训练）"""
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="MambaGlue CFFA Real-Data Testing")
...
def main():
    ...
    # 从检查点加载模型
    model = PL_MambaGlue_Real.load_from_checkpoint(
        str(ckpt_path),
        config=config,
