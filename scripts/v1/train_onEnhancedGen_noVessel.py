...
# 导入域随机化模块
from scripts.v1.gen_data_enhance import random_domain_augment_pair
...
class GenDatasetWrapper(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.apply_domain_randomization:
            # 对 fix 和 moving 分别独立应用域随机化
            # 这会破坏纹理捷径，迫使模型学习几何特征
            image0, image1 = random_domain_augment_pair(image0, image1)
        
        # 构建返回结果
        result = {
            'image0': image0,                  # [1, H, W] CF (固定图，可能已增强)
            'image1': image1,                  # [1, H, W] FA deformed (未配准的移动图，可能已增强)
            'image1_gt': data['image0'],       # [1, H, W] 使用原始 CF 作为配准目标（不增强）
...
class MultimodalDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            self.train_dataset = GenDatasetWrapper(train_base, apply_domain_randomization=True)  # 启用域随机化
...
# ==========================================
def parse_args():
    parser = argparse.ArgumentParser(description="MambaGlue Gen-Data Training with 260227_2_v29_2_1 Dataset")
...
def main():
    ...
    logger.info("域随机化: 已启用 (训练时对图像对独立应用域增强)")
    ...
    logger.info(f"开始生成数据训练 (训练集: 260227_2_v29_2_1 生成数据 + 域随机化 | 验证集: CFFA 真实数据): {args.name}")
