...
class PL_MambaGlue_Gen(pl.LightningModule):
    ...
    def _compute_loss(self, outputs, kpts0, kpts1, T_0to1, smoothing=0.1):
        """计算负对数似然损失（带 Label Smoothing）"""
        ...
        nll_loss = -target_log_probs.mean()
        
        # Label Smoothing: 添加均匀分布的损失
        smooth_loss = -scores[:, :M, :].mean()
...
def main():
    ...
    logger.info("早停配置: monitor=combined_auc, start_epoch=0, patience=20, min_delta=0.0001")
    ...
    logger.info(f"开始生成数据训练 (训练集: 260227_2_v29_2_1 生成数据 | 验证集: CFFA 真实数据): {args.name}")
