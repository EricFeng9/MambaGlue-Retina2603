...
def compute_homography_errors(data, config):
    ...
    pts1 = data['mkpts1_f'].cpu().numpy() if torch.is_tensor(data['mkpts1_f']) else data['mkpts1_f']
    H_gt = data['T_0to1'].cpu().numpy() if torch.is_tensor(data['T_0to1']) else data['T_0to1']
    mconf = data.get('mconf')
    if mconf is not None:
        mconf = mconf.cpu().numpy() if torch.is_tensor(mconf) else mconf
    ...
    for bs in range(H_gt.shape[0]):
        ...
        if inliers_rate < 1e-6:
            _dual_log("WARNING", f"⚠️ Batch {bs}: inliers rate < 1e-6, 判定为 Failed")
            is_failed = True
        if is_identity:
