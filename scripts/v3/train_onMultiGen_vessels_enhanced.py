...
# 导入域随机化增强模块
from scripts.v3.gen_data_enhance import random_domain_augment_image
...
def parse_args():
    ...
    parser.add_argument('--name', '-n', type=str, default='mambaglue_vessel_guided_v3', help='训练名称')
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("【MambaGlue 血管引导训练 - 域随机化增强版】")
    logger.info("=" * 80)
    ...
    logger.info(f"域随机化增强: 已启用 (gen_data_enhance.py)")
    logger.info("=" * 80)
    ...
    logger.info("策略: 血管loss引导的课程学习 + 域随机化增强")
